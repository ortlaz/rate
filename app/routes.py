...
def account(page=1):
    if current_user:
        ...
        rates = db.session.query(Rate_formula).filter_by(author_id=current_user.get_id()).paginate(page, app.config[
            'POSTS_ON_PAGE'], False)
...
def main(page=1):
    ...
    ratings_lst = db.session.query(Rate_formula).paginate(page, app.config['POSTS_ON_PAGE'], False)
...
def save_file():
    ...
    if request.method == 'POST':
        ...
        if dl_file and legal_files(dl_file.filename):
            ...
            if current_user:
                f_name = secure_filename(f_name_list[0] + '_' + current_user.get_id() + '_' + datetime.now().strftime(
                    "%Y-%m-%d_%H-%M-%S") + '.' + f_name_list[1])
...
def dowload_file(filename):
    return send_from_directory(app.config['FOLDER_FOR_FILES'], app.config['FOLDER_FOR_FILES'] + '/' + filename)
...
def chooseparams():
    ...
    if request.method == "POST":
            ...
            if data[i]['formula'] == '':
                if data[i]['name'] not in parameters:
                    error_no_param = 'Ошибка! Нет параметра\n' + data[i][
                        'name'] + '. Загрузите новый файл или выберите другие параметры'
        ...
        first_id = db.session.query(func.max(Params.id)).scalar()  # начало диапазона созданных парметров
        ...
        last_id = db.session.query(func.max(Params.id)).scalar()  # конец диапазона созданных парметров
...
def finish():
    ...
    if request.method == "POST":
        ...
        fl_id = db.session.query(Rate_formula).filter_by(author_id=current_user.get_id()).order_by(
            Rate_formula.id.desc()).first().id
...
def make_rate():
    ...
    if session.get('id_build') != -1:
        ...
        fl = db.session.query(Rate_formula).filter_by(author_id=current_user.get_id()).order_by(
            Rate_formula.id.desc()).first()
